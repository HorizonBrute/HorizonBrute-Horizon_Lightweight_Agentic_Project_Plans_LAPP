...
edits, and per-folder tweaks would leak globally. The Horizon.AIOS already uses a `*.template` → `*.local.*`
...
3. **Project/folder-specific override** — a `PROJECT_PLAN_GUIDE.local.md` in a folder, referenced from
   that folder's `agents.md`, amending the base for that folder only. Needs no Horizon.AIOS.
...
- Standalone (non-Horizon.AIOS) users get tiers 1 and 3; the admin tier is Horizon.AIOS-only (needs `$HORIZON_ETC`).
