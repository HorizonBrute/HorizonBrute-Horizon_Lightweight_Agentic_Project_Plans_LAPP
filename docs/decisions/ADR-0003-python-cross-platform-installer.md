...
- Matches AIOS Python tooling conventions while staying clearly outside the OS namespace.
- Idempotent operations (re-runnable install, marker-delimited context injection, upsert registry) make
  updates and repair safe.
- Requires Python 3.8+ on the target (already a given in an AIOS environment).
